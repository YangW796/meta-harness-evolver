        ...
        for h in genes_to_search:
            ...
            try:
                ...
                similar_available = [i for i in similar_indices if i in available_indices and i not in exploit_pool and i not in selected]
                                 # Adaptive quality filtering based on hit rate
                                # If we have many hits (high hit rate), be more selective (keep fewer, higher quality)
                                # If we have few hits (low hit rate), keep more diverse candidates to explore
                                hit_rate = len(hit_genes) / len(history) if history else 0
                                # Map hit rate to keep percentage: 30% at high hit rate (>=0.1), 80% at low hit rate (0)
                                # Linear interpolation between these points
                                if hit_rate >= 0.1:
                                    keep_fraction = 0.3
                                else:
                                    keep_fraction = 0.3 + (0.1 - hit_rate) * 5  # 0.3 to 0.8
                                keep_count = max(1, int(len(similar_available) * keep_fraction))                exploit_pool.extend(similar_available[:keep_count])
            except:
                pass

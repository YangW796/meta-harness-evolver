...
def select(candidates, history, batch_size, seed) -> list[int]:
    ...
    explore_indices = rng.sample(available, min(num_explore, len(available)))
            ...
            if len(history) > 0 and num_exploit > 0:
                ...
                # Calculate score-based weights for remaining candidates
                # Bias toward candidates with indices near top performers
                exploit_candidates = set()
                remaining_avail = [i for i in available if i not in explore_indices]
                
                if len(remaining_avail) > 0:
                    # Create weights based on proximity to top performers
                    # Candidates near good performers are more likely to be good
                    weights = np.ones(len(remaining_avail))
                    
                    if len(top_performers) > 0:
                        for idx, cand_idx in enumerate(remaining_avail):
                            # Calculate minimum distance to any top performer
                            min_dist = min(abs(cand_idx - top_idx) for top_idx in top_performers)
                            # Weight inversely proportional to distance (closer = higher weight)
                            weights[idx] = 1.0 / (1.0 + min_dist / 100.0)
                ...
                selected_indices = list(explore_indices) + list(exploit_candidates)    else:

...
export PROPOSER_BACKEND="${PROPOSER_BACKEND:-nexau}"
...
export PROPOSER_ATTEMPTS_PER_CANDIDATE=5

PROJECT_BDA_TRADITIONAL_MODE="${PROJECT_BDA_TRADITIONAL_MODE:-0}"
if [[ "$PROJECT_BDA_TRADITIONAL_MODE" == "1" ]]; then
  export EVOLVER_PROPOSER_MODE="traditional"
  export EVOLVER_TRADITIONAL_MODEL_PATH="${EVOLVER_TRADITIONAL_MODEL_PATH:-$PROJECT_DIR/tradition_model.py}"
  export PROPOSER_ATTEMPTS_PER_CANDIDATE=1
  export PROPOSER_PLAN_ATTEMPTS=0
  export BDA_TRADITIONAL_POLICY="${BDA_TRADITIONAL_POLICY:-random}"
fi

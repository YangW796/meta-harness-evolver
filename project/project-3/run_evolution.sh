...
if [[ ! -d "$PROJECT3_WORKSPACE_DIR" ]]; then
  mkdir -p "$PROJECT3_WORKSPACE_DIR"
fi

if [[ -d "$PROJECT3_SEED_BEST_DIR" && ! -d "$PROJECT3_WORKSPACE_DIR/best/current" ]]; then
  mkdir -p "$PROJECT3_WORKSPACE_DIR/best"
  cp -a "$PROJECT3_SEED_BEST_DIR/." "$PROJECT3_WORKSPACE_DIR/best/"
fi
...
export EVOLVER_WORKSPACE="$PROJECT3_WORKSPACE_DIR"
...
export NEXAU_DENY_RUN_SHELL_SUBSTRINGS="index.py"
...
ARGS=(python -u "$EVOLVER_ROOT/scripts/run_evolution.py" --workspace "$PROJECT3_WORKSPACE_DIR" --iterations "$ITERATIONS" --evaluate-script "$EVALUATE_SCRIPT_PATH")
